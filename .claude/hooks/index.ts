...
const sessionStart: SessionStartHandler = async (payload) => {
	...
	console.error(`🚀 New session started from: ${payload.source}`);
	console.error(`📍 Session ID: ${payload.session_id}`);
	...
	if (payload.source === "vscode") {
		console.error(
			"👨‍💻 VS Code session detected - enabling IDE-specific features",
		);
	} else if (payload.source === "web") {
		console.error("🌐 Web session detected");
	}
...
const preToolUse: PreToolUseHandler = async (payload) => {
	...
	// Block the word "sonos" in TypeScript file edits
	if (
		(payload.tool_name === "Edit" || payload.tool_name === "Write") &&
		payload.tool_input
	) {
		const filePath =
			(payload.tool_input as { file_path?: string }).file_path ?? "";
		if (filePath.endsWith(".tsx") || filePath.endsWith(".ts")) {
			const content =
				(payload.tool_input as { new_string?: string }).new_string ??
				(payload.tool_input as { content?: string }).content ??
				"";
			if (/sonos/i.test(content)) {
				console.error(
					'❌ Blocked: the word "sonos" is not allowed in .tsx files.',
				);
				return {
					permissionDecision: "deny",
					permissionDecisionReason:
						'The word "sonos" is not allowed in .tsx files.',
				};
			}
		}
	}
